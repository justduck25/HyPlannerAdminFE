...
export const dashboardAPI = {
  ...
  getUserGrowthByMonth: (months = 12) => apiClient.get(`/dashboard/user-growth-monthly?months=${months}`),
  getOverview: () => apiClient.get('/dashboard/overview'),
  getAccountDistribution: () => apiClient.get('/dashboard/account-distribution'),
  getSystemHealth: () => apiClient.get('/dashboard/system-health'),
  getRecentActivities: (limit = 10) => apiClient.get(`/admin/recent-activities?limit=${limit}`),
};
...
export const paymentsAPI = {
  getPayments: (params) => Promise.resolve({ data: { payments: [], pagination: { total: 0 } } }),
